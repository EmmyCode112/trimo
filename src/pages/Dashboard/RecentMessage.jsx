...
export default function RecentMessage() {
  ...
  const filters = isMobile ? ["12m", "30d", "7d"] : ["12 months", "30 days", "7 days", "+ Custom"];
      ...
      <div className="flex justify-between items-center mt-4">
        <div className="flex space-x-2">
          {filters.map((filter) => (
            ...
            >
              {filter}
              {filter === "+ Custom" && showCalendar && (
                <div className="absolute bottom-[-80px] z-[10]">
                  <Calendar onChange={handleDateChange} tileDisabled={disableFutureDates} />
                </div>
              )}
            </button>
          ))}
